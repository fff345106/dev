...
class SpecialEventControllerTest {
    ...
    @BeforeEach
    void setUp() {
        mockMvc = MockMvcBuilders.standaloneSetup(new SpecialEventController(specialEventService))
    ...
    void delete_shouldCallServiceAndReturnOk() throws Exception {
        mockMvc.perform(delete("/api/events/{id}", 5L))
                .andExpect(status().isOk());

        verify(specialEventService).delete(5L);
    }
}
